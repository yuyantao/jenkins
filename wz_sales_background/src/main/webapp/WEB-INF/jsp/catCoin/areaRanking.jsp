	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody id="listData">
				...
				<c:forEach items="${lists}" var="list" varStatus="ind" >
				<c:set var="listNumber" value="${ind.index+1}"></c:set>
				<tr class="text-c">
					<td><c:if test="${fn:length(lists) > ind.count}">${ind.count}</c:if></td>
					<td>${list.area}</td>
...
function loadData(page){
	$("#page").val(page);
	pagingList({
		url:"${base}/areaRanking/list",//访问地址，必填
...
function haha(){
	$("#queryForm").attr("action","${base}/areaRanking/downlodCSV");
	$("#queryForm").submit();
}
